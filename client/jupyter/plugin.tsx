...
import {
  ILoggerRegistry,
  LogLevel,
} from "@jupyterlab/logconsole"; /* @ts-ignore */
import { IRenderMimeRegistry } from "@jupyterlab/rendermime"; /* @ts-ignore */
import { PageConfig } from "@jupyterlab/coreutils"; /* @ts-ignore */
import {
  MainAreaWidget,
  WidgetTracker,
} from "@jupyterlab/apputils"; /* @ts-ignore */
import {
  INotebookTracker,
  Notebook,
  NotebookPanel,
 ...
 */
function* getLinkedWidgetsFromApp(jupyterApp: JupyterFrontEnd, path: string) {
      ...
      const callback = (outputArea, outputChanges) => {
        for (let index = 0; index < notebook.model.cells.length; index++) {
          if (cell === notebook.model.cells.get(index)) {
            const detachPret = outputChanges.newValues.some(
              (outputModel) => !!outputModel._rawData?.[MIMETYPE]?.["detach"]
            );
            if (detachPret) {
...
async function activatePretExtension(
  ...
  if (notebookTracker !== null) {
    ...
    notebookTracker.currentChanged.connect((sender, panel: NotebookPanel) => {
      registerOutputListener(panel.content, (path, index) => {
        (commands as CommandRegistry).execute("pret:create-view", {
          path,
          index,
        });
  ...
  if (menu) {
    menu.settingsMenu.addGroup([{ command: "pret:saveAnnotatorState" }]);
  }
  ...
  commands.addCommand("pret:open-page", {
    label: "⚛️ Open in a new browser tab",
    execute: async (args) => {
      ...
      const url = new URL(
        PageConfig.getUrl({
          ...
          treePath: current.context.path,
        }),
        window.location.origin
      );
      url.searchParams.set("pret-fullpage-cell", `${index}`);
  ...
  commands.addCommand("pret:create-view", {
    ...
    execute: async (args) => {
      ...
      const hasBeenDetached = !!pretAreas.find(
        (widget) =>
          widget.content.path === path && widget.content.index === index
      );
      ...
      await Promise.all([commands.execute("notebook:hide-cell-outputs", args)]);
    },
    isEnabled: isEnabledAndSingleSelected,
  });
